 ...
 *
 * The transport itself (client.ts) is covered by a smoke script against the
 * real API — per AGENTS.md a smoke run is sufficient there, and mocking the SDK
 * would only test the mock.
 *
 * Runs under `node --test test/llm.test.ts` using Node's native type stripping,
 * so this file stays erasable-syntax only: no enums, no parameter properties,
 * no namespaces.
 *
 * Relative specifiers carry an explicit `.ts` extension because the runtime —
 * unlike the bundler that resolves lib/ — will not resolve an extensionless or
 * `.js`-rewritten path to a `.ts` file; both were tried and both fail to load.
 * The cost is that `tsc` flags these two imports (TS5097) until the root
 * tsconfig sets `"allowImportingTsExtensions": true`, which is safe here since
 * `noEmit` is already on. Runtime correctness wins: this file exists to be run.
 */
...
import {
  ...
  describeLlmConfig,
  DEFAULT_MODEL,
} from '../lib/server/llm/config.ts';
import type { LlmEnv } from '../lib/server/llm/config.ts';
import { mapWithConcurrency, partitionResults } from '../lib/server/llm/pool.ts';
import type { SettledResult } from '../lib/server/llm/pool.ts';

const baseEnv: LlmEnv = { OPENROUTER_API_KEY: 'sk-or-v1-testkeyvalue' };
...
describe('mapWithConcurrency', () => {
  it('returns results in input order regardless of completion order', async () => {
    ...
    // The union has no common `value`, so unwrap through the discriminant. A
    // failed slot would surface its error string and fail the comparison, which
    // is the same signal the JS original gave.
    assert.deepEqual(
...
describe('partitionResults', () => {
  it('keeps original indices so failures are identifiable', () => {
    const settled: Array<SettledResult<string>> = [
      { ok: true, value: 'a' },
      { ok: false, error: 'boom' },
      { ok: true, value: 'c' },
    ];

    const { succeeded, failed } = partitionResults(settled);
