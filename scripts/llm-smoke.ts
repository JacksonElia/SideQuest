...
import { complete, completeMany } from '../lib/server/llm/index.ts';
import { loadLlmConfig, describeLlmConfig } from '../lib/server/llm/config.ts';
...
main().catch((err: unknown) => {
  ...
  console.error(`[llm:smoke] fatal: ${err instanceof Error ? err.message : String(err)}`);
  process.exitCode = 1;
});
