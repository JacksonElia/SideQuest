...
import { validateFix, encodeLocationAttributes } from './location.ts';
import type { Fix, TimedFix } from './location.ts';
...
export function newRoomName(prefix = 'sidequest'): string {
...
export async function mintToken(
  cfg: LiveKitConfig,
  { roomName, identity, fix }: { roomName: string; identity: string; fix?: TimedFix },
): Promise<string> {
...
export async function dispatchAgent(
  cfg: LiveKitConfig,
  { roomName, fix }: { roomName: string; fix?: TimedFix },
) {
...
/** Error carrying an HTTP status, so the route handler can map it to a response. */
...
export async function createSession(
  cfg: LiveKitConfig,
  { fix: rawFix, identity }: { fix: Partial<Fix>; identity?: string },
): Promise<Session> {
  const validated = validateFix(rawFix);
  if (!validated.ok) {
    throw new HttpError(validated.error, 400);
  }
  ...
  try {
    ...
    console.error(`[session] dispatch failed for room=${roomName}: ${dispatchError}`);
  }
