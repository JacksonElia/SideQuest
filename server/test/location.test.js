...
test('validateFix carries through a non-negative accuracy', () => {
  ...
  assert.equal(r.value.accuracy, 12.5);
});
...
test('encodeLocationAttributes returns only string values', () => {
  const attrs = encodeLocationAttributes({ lat: 37.7818, lng: -122.3927, accuracy: 12.5, ts: 1720000000000 });
...
test('encodeLocationAttributes keeps discrete numeric fields, not a sentence', () => {
  ...
  assert.ok(!/\b(near|at|you are|located)\b/i.test(joined), 'coordinates must not be embedded in prose');
});
